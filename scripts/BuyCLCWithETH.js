...
async function main() {
  ...
  while (numOfTxSent < NUMBER_OF_TX) {
    const buyData = await exchange.methods
        .buy(ETH_TOKEN_ADDRESS, CLC_ADDRESS, USER_ADDRESS, Web3Utils.toWei(tradeTokenAmount.toString()), Web3Utils.toWei(tradePrice.toString()))
        .encodeABI({ from: USER_ADDRESS })

    const txData = await erc827.methods
